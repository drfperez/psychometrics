Aquí teniu el text complet del README, formatat correctament amb LaTeX per als símbols matemàtics, llest per copiar i enganxar directament a un fitxer README.md a GitHub:
...
Interpretació: r_{pb} \ge 0{,}20 és desitjable; valors r_{pb} \le 0 poden indicar incoherència.
Càlcul en R:
Pbis_Q1 <- res_CTT$itemReport["Q1", "pBis"]

4. Fiabilitat (Alfa de Cronbach)
Definició: Consistència interna del conjunt d’ítems.
Fórmula: \alpha = \frac{k}{k-1} (1 - \frac{\sum_{i=1}^k \sigma_i^2}{\sigma_X^2}), amb k el nombre d’ítems, \sigma_i^2 la variància de l’ítem i i \sigma_X^2 la variància de la puntuació total.
Interpretació:
